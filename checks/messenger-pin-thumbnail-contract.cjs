...
const filename = path.resolve(__dirname, "../services/messengerPinVideoThumbnail.ts");
const players = [];
let options = {};
const native = {
  createVideoPlayer(source) {
    ...
    const player = {
      status: "readyToPlay", calls: [], released: 0, removed: 0,
      ...
      replaceAsync(source) {
        this.source = source;
        return behavior.replaceFails ? Promise.reject(new Error("offline")) : Promise.resolve();
      },
      generateThumbnailsAsync(times, size) {
        ...
        if (!Array.isArray(times)) throw new TypeError("iOS SDK 54 requires [Double]");
        if (behavior.generateFails) return Promise.reject(new Error("invalid video"));
...
(async () => {
  const frames = [];
  const cancel = request("/fixture.mp4", { Authorization: "Bearer test-only" }, (frame) => frames.push(frame));
  await tick();
  const first = players[0];
  assert.deepEqual(first.calls, [{ times: [0], size: { maxWidth: 160, maxHeight: 160 } }],
  ...
  assert.equal(first.calls.length, 1, "one generation per request despite duplicate ready events");
  ...
  const beforeLoad = request("/cancel-before-load", undefined, (frame) => frames.push(frame));
  beforeLoad();
  await tick();
  assert.equal(players[1].calls.length, 0);
  assert.equal(players[1].released, 1);

  const duringGeneration = request("/cancel-after-load", undefined, (frame) => frames.push(frame));
  await tick();
  duringGeneration();
  players[2].finish([{ nativeRefType: "image" }]);
  await tick();
  assert.equal(frames.length, 1, "late frame never updates an unmounted banner");
  ...
  assert.equal(frames.length, 1);
  console.log("Pin thumbnail native-array contract, mute, authorization, duplicate events, cancellation and media errors passed.");
