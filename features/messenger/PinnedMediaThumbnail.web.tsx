...
export default function PinnedMediaThumbnail({ media, accessToken, active }: PinnedMediaThumbnailProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void cacheMessengerMedia(mediaRef.current, accessToken).then((uri) => {
  ...
  return (
    <View style={styles.box} pointerEvents="none" accessible={false}>
      ...
      {uri && (media.type === "image" ? <Image source={uri} style={StyleSheet.absoluteFill} contentFit="cover" /> :
        <video src={uri} muted playsInline preload="auto" controls={false} aria-hidden="true" tabIndex={-1}
...
const styles = StyleSheet.create({ box: { width: 48, height: 48, borderRadius: 6, overflow: "hidden", alignItems: "center", justifyContent: "center", marginRight: 10, backgroundColor: "#EAF2FA" } });
