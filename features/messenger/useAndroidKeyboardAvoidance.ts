...
const MEASUREMENT_DELAYS_MS = [0, 80, 220, 420] as const;
const METRICS_PROBE_DELAYS_MS = [0, 90, 220, 450, 800] as const;
...
export function useAndroidKeyboardAvoidance(
  ...
  const measureOverlap = useCallback(() => {
    ...
    target.measureInWindow((_x, y, _width, height) => {
      ...
      if (!activeFrame || height <= 0) return;
      updateInset(
        calculateAndroidKeyboardInset({
          targetBottom: y + height,
          appliedInset: appliedInsetRef.current,
          keyboardScreenY: activeFrame.screenY,
          keyboardHeight: activeFrame.height,
  ...
  const onTargetLayout = useCallback(
    (_event: LayoutChangeEvent) => {
      if (Platform.OS !== "android" || !keyboardFrameRef.current) return;
  ...
  return { bottomInset, onTargetLayout, refresh };
}
