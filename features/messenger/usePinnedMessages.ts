...
import { getMessengerPins, setMessengerMessagePinned } from "../../services/messengerPins";
import { subscribeMessengerRealtime } from "../../services/messengerRealtime";
import { orderedMessengerPins, type MessengerPin } from "./pins";

interface PinSnapshot { key: string; items: MessengerPin[]; canPin: boolean }
const empty: MessengerPin[] = [];

/** Shared pins belong to the server; the selected banner is local to this screen. */
export function usePinnedMessages(roomId: string, userId: string | undefined, active: boolean) {
  const key = `${userId ?? ""}:${roomId}`;
  const [snapshot, setSnapshot] = useState<PinSnapshot>({ key: "", items: [], canPin: false });
  const [mutation, setMutation] = useState<{ key: string; id: string } | null>(null);
  ...
  useEffect(() => {
      ...
      running = (async () => {
        do {
          ...
          try {
            ...
            if (!cancelled && !dirty) setSnapshot({ key, items: orderedMessengerPins(result.items), canPin: result.can_pin });
          } catch (error) {
            if (!cancelled && error instanceof MessengerApiError && [401, 403, 404].includes(error.status)) {
    ...
    const unsubscribe = subscribeMessengerRealtime((event) => {
      if (event.type === "connection.ready" || event.type === "sync.required" ||
          (event.type === "connection.state" && event.connected) ||
          ((event.type === "room.pins_updated" || event.type === "room.updated") && event.room_id === roomId) ||
          ((event.type === "message.created" || event.type === "message.updated") &&
            event.message.room_id === roomId && (event.message.kind === "system" || pinIds.current.has(event.message.id)))) {
        void refresh();
      }
    });
    ...
    return () => {
      cancelled = true;
      unsubscribe(); appState.remove(); clearInterval(timer);
  ...
  const setPinned = useCallback(async (messageId: string, pinned: boolean) => {
    ...
    if (!active || !canPin) throw new MessengerApiError("Нет права закреплять сообщения в этом чате", 403, "chat_write_denied");
    const token = Symbol();
    ...
    try {
      ...
      if (mutationToken.current === token) { mutationToken.current = null; setMutation(null); }
    }
