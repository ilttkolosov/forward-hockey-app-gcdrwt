...
export function useMediaViewerLoading(items: readonly MessengerMedia[], index: number | null, session: number, ensureLocal: (item: MessengerMedia) => Promise<string>): void {
  const ensureRef = useRef(ensureLocal);
  ensureRef.current = ensureLocal;
  const visit = useRef<{ key: string; requested: Set<string> }>({key: "", requested: new Set()});
  ...
  useEffect(() => {
    ...
    if (visit.current.key !== key) visit.current = {key, requested: new Set()};
