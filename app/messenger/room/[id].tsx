...
function messageReplyPreview(reply: MessengerReply): string {
  ...
  if (reply.text) return stripMessengerTextFormatting(reply.text);
...
function messageProbablyNeedsCollapse(text: string): boolean {
  return (
    stripMessengerTextFormatting(text).length >
      COLLAPSED_MESSAGE_ESTIMATED_CHARACTERS ||
...
function ExpandableMessengerMessageText({
  ...
  const [longMessage, setLongMessage] = useState(() =>
    messageProbablyNeedsCollapse(text),
  );
  ...
  return (
    <View>
      <MessengerLinkifiedText
        ...
        style={style}
        numberOfLines={
          longMessage && !expanded ? COLLAPSED_MESSAGE_LINES : undefined
        }
        onTextLayout={(event) => {
          if (
            !longMessage &&
      ...
      {longMessage ? (
        <Pressable
          ...
          accessibilityLabel={
            expanded ? "Свернуть сообщение" : "Показать сообщение полностью"
          }
          hitSlop={6}
        >
          <Text style={styles.messageExpandLink}>
          ...
          : item.kind === "video"
            ...
            : item.kind === "file"
              ...
              : item.kind === "location"
                ...
                : ""
        : "");
  const plainBody = stripMessengerTextFormatting(body);
  const emojiOnly =
    !item.deleted_at && item.kind === "text" && isEmojiOnly(plainBody);
  const linkPreviewUrl = item.deleted_at
    ? null
    : firstMessengerMessageUrl(plainBody);
    ...
    <View collapsable={false}>
      ...
      >
        ...
        >
          ...
          <View
            style={[styles.messageColumn, mine && styles.messageColumnMine]}
          >
            <View
              style={[
                styles.message,
...
export default function MessengerRoomScreen() {
  ...
  const [roomCanManage, setRoomCanManage] = useState(
    params.canManage === "true",
  );
  ...
  const loadMessages = useCallback(
    async (initial = false) => {
      ...
      try {
        ...
        if (initial && !localSnapshotReady && params.pushMessageId && session) {
    ...
    ({ viewableItems }: { viewableItems: ViewToken<MessengerMessage>[] }) => {
      ...
      if (firstVisible) {
        const currentIndex = messagesRef.current.findIndex(
          (message) =>
            message.client_message_id === firstVisible.item.client_message_id,
        );
        const previous =
  ...
  const chooseAttachment = useCallback(
    async (request: PendingAttachmentRequest) => {
      ...
      try {
        ...
        // Start the iOS file read behind the attachment preview. If the user
        // sends immediately, sendBufferedMessengerMedia awaits this same work;
        // if they add a caption, the expensive first read is already hidden.
        void warmMessengerBufferedUploadFiles(files);
  ...
  const copyMessageText = useCallback(async (message: MessengerMessage) => {
    ...
    await Clipboard.setStringAsync(stripMessengerTextFormatting(message.text));
    setActionMessage(null);
    setShowAllReactions(false);
  }, []);
    ...
    async (message: MessengerMessage) => {
      ...
      try {
        ...
        Alert.alert(
          ...
          messengerErrorMessage(
            error,
            "Этот пользователь недоступен для личной переписки",
          ),
        );
      }
    },
                ...
                <>
                  ...
                  >
                    {forwardError ? (
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          forwardingMessage &&
                          void openForward(forwardingMessage)
                        }
                      >
                        <Icon
                  ...
                  <View style={styles.forwardFooter}>
                    <TouchableOpacity
                      ...
                      disabled={
                        forwardSelectedKeys.size === 0 || Boolean(forwardBusy)
                      }
                      accessibilityRole="button"
          ...
          >
            ...
            {editingMessage && (
              <View style={styles.editComposer}>
                <View style={styles.replyComposerText}>
                  ...
                  <Text style={styles.replyText} numberOfLines={1}>
                    {stripMessengerTextFormatting(editingMessage.text) ||
                      "Подпись к вложению"}
                  </Text>
                </View>
                <TouchableOpacity
            ...
            <View style={styles.composer}>
              ...
              <ForwardRichTextInput
                ...
                selection={
                  Platform.OS === "web" ? composerSelection : undefined
                }
                onSelectionChange={
